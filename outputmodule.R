...
output_ui <- function(id) {
	...
	fluidRow(
		...
		column(6,
			wellPanel(
				sliderInput(ns("pdf_width"), "Plot File Page Width", min = 3, max = 30, step = 1, value = 12),
				sliderInput(ns("pdf_height"), "Plot File Page Height", min = 3, max = 50, step = 1, value = 8),
				actionButton(ns("clear_saved_plots"), "Clear all saved plots"),
				tags$br(),
				checkboxGroupInput(ns("plots_checked"), "Plots to Save", choices=NULL, selected=NULL),
				downloadButton(ns('downloadPDF'), 'Download PDF'),
				downloadButton(ns('downloadSVG'), 'Download SVG (for the first selected plot)'),
				tags$br(),tags$hr(),
				downloadButton(ns('downloadXLSX'), 'Download tables in .xlsx')
			)
		),
		column(3,
		)
	...
	shiny::moduleServer(id,
		function(input, output, session) {
			...
			output$downloadPDF <- downloadHandler(
				...
				content = function(file) { withProgress(message = 'This takes a minute or two',{
					...
					if (!is.null(saved_plots$SampleDistance) & ("Sample Distance Plot" %in% plots_checked) ){
						...
						draw(saved_plots$SampleDistance, merge_legend=T, auto_adjust = FALSE)
						Np=Np+1
					}
					...
					### AbundanceCurve
					if (!is.null(saved_plots$AbundanceCurve)){
						print(saved_plots$AbundanceCurve)
						Np=Np+1
					}
					...
					###upsetsplot
					if (!is.null(saved_plots$upsetsplot)){
						for (i in 1:length(saved_plots$upsetsplot)) {
							title_tmp <- paste("upsetsplot (",i,")",sep="")
							if (title_tmp %in% plots_checked) {
								draw(saved_plots$upsetsplot[[i]], merge_legend=T, auto_adjust = FALSE)
								Np=Np+1
							}
						}
					}
			...
			observe({
				...
				##AbundanceCurve
				if (!is.null(saved_plots$AbundanceCurve)){
					summary=c(summary, 'Rank Abundance Curve')
				}
			...
			output$downloadSVG <- downloadHandler(
				...
				content = function(file) { withProgress(message = 'This takes a minute or two',{
					...
					### AbundanceCurve
					if (Np==0 & !is.null(saved_plots$AbundanceCurve)){
						print(saved_plots$AbundanceCurve)
						Np=Np+1
					}
			...
			output$downloadXLSX <- downloadHandler(
				...
				content = function(file) { withProgress(message = 'This takes a minute or two',{
					...
					for (i in names(saved_table)) {
						...
						writeData(wb, i, res_table, rowNames = FALSE)
					}

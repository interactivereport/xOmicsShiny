...
The merged output can be considered as a new comparison, and it does not have raw data values. Thus, after merging the comparison results, users can apply the modules that don't require raw data to this merged result. For example, users can visualize the merged output using many functions that described previously in the tutorial, such as the aforementioned DEG visualization in **Chapter \@ref(deg-module)**, Venn diagram in **Chapter \@ref(venn-diagram-module)**, and pathway enrichment analysis in **Chapter \@ref(gene-set-enrichment-module)**, etc.

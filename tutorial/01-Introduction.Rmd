...
- **Easy comparison of DE results**
  ...
  xOmicsShiny has several ways to examine the differential expression (DE) results and compare them directly. For example, as described in Chapter \@ref(degs-and-deps-in-two-comparisons) and \@ref(mulitple-volcano-plots), scatter plots and side-by-side volcano plots are frequently used.
  
- **Comparison result merging**

  Instead of merging the original data, xOmicsShiny features multi-omics data integration at the comparison level, as illustrated in Chapter \@ref(comparison-result-merging-module). The merged result will be treated as a new dataset for visualization.
